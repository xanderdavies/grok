...
def get_train_logdict(train_acc, train_loss, loss_with_reg, loss_normalized, epoch, train_step_counter):
    log_dict = {
...
def get_interpolation_logdict(epoch_train_acc, epoch, train_step_counter, interpolated_99, interpolated_100):
    log_dict = dict()
    if epoch_train_acc > .99 and not interpolated_99:
...
def validate(args, model, train_dataloader, val_dataloader, zero_enc, one_enc, criterion, epoch, train_step_counter, generalized_99, generalized_100, generalized_90):
...
def run(args: argparse.Namespace):
    ...
    if args.resume_run_id is None:
        wandb.init(entity="initially-overconf-learners", project=args.wandb_project, id=date_time, settings=wandb.Settings(start_method="thread"), tags=tags, name=name, config=args, mode=mode)
    else:
        wandb.init(id=args.resume_run_id, resume="must", project=args.wandb_project, settings=wandb.Settings(start_method="thread"), tags=tags, name=name, config=args, mode=mode)
    wandb.watch(model)
    ...
    reg_criterion = lambda y_hat, target, *args, **kwargs: reg_cel(y_hat[:, -2, :], target[:, -2])
    ...
    log_dict = validate(args, model, train_dataloader, val_dataloader, zero_enc, one_enc, criterion, 0, 0, generalized_99, generalized_100, generalized_90)
    wandb.log(log_dict)
    ...
    for epoch in tqdm(range(1, int(OPTIMIZATION_BUDGET / steps_per_epoch) + 1)):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            step_data = train_step(args, model, optimizer, reg_criterion, criterion, batch)
            epoch_data.append(step_data)

            if train_step_counter < 1000 and i != len(train_dataloader) - 1:
                log_dict = validate(args, model, train_dataloader, val_dataloader, zero_enc, one_enc, criterion, epoch, train_step_counter, generalized_99, generalized_100, generalized_90)
        ...
        log_dict = validate(args, model, train_dataloader, val_dataloader, zero_enc, one_enc, criterion, epoch, train_step_counter, generalized_99, generalized_100, generalized_90)
        log_dict.update(get_train_logdict(*step_data, epoch, train_step_counter))
        log_dict.update(get_interpolation_logdict(step_data[0], epoch, train_step_counter, interpolated_99, interpolated_100))
        wandb.log(log_dict)
